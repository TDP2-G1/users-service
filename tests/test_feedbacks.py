...
class FlaskTest(unittest.TestCase):
    ...
    def test_valid_feedback(self):
        ...
        response = tester.post("/feedback/", data=VALID_FEEDBACK, content_type='application/json')
    ...
    def test_valid_feedback_get(self):
        tester = create_app().test_client(self)
        genre_creation(tester, 'Male')
        language_creation(tester, 'English')
        level_creation(tester, 'Advanced')
        ...
        response = tester.post("/user/", data=ANOTHER_VALID_REGISTER, content_type='application/json')
        response = tester.post("/feedback/", data=VALID_FEEDBACK, content_type='application/json')
        response = tester.get("/feedback/1", content_type='application/json')
        status_code = response.status_code
        data = json.loads(response.get_data(as_text=True))
        print(data)
        self.assertEqual(data[0]['id_user_giver'], 2)
        self.assertEqual(status_code, 200)
